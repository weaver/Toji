...
function assertValidPrim(data) {
  if (!this.isValid(data))
    throw new Error('expected `' + this.__name__ + '`, not ' + show(data));
  return data;
}
...
RecordType.assertValid = function(data) {
  if (!this.isValid(data))
    throw new Error(this.__name__ + ': expected object, not ' + show(data));
  return data;
};
...
Field.prototype.assertValid = function(data) {
  if (!this.isValid(data))
    throw new Error(this + ': expected `' + this.type.__name__ + '`, not ' + show(data));
  return data;
}
...
Field.prototype.load = function(data) {
  ...
  return this.type.load(this.assertValid(data));
};

Field.prototype.dump = function(obj) {
  return this.type.dump(this.assertValid(obj));
};
...
ArrayType.assertValid = function(data) {
  if (!this.isValid(data))
    throw new Error(this.__name__ + ': expected Array, not ' + show(data));
  return data;
};
