...
RecordType.include({
  ...
  attr: function(data) {
    ...
    Type.of(this).eachExisting(data, function(val, name, field) {
      if (field.schema.writable !== false && !field.schema.protected)
        self[name] = val;
    });
    return this;
  },
...
RecordType.extend({
  ...
  loadJSON: function(obj) {
    var type = this;
    return U.extend(new type(), this.mapRecursive(this.assertValid(obj), 'loadJSON'));
  },
  ...
  exportJSON: function(obj) {
    return this.mapRecursive(obj, 'exportJSON', function(obj, name, field) {
      return (field.schema.readable !== false && !field.schema.protected);
    });
  }
});
...
function recordCtor(name) {
  ...
  with (env) {
    eval('$ctor = function ' + name + '(obj){ obj && this.attr(obj); }');
  }
