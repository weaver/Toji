...
function defPrim(name, methods) {
  return (exports.TYPES[name] = Schema.createType(name, primCtor(name)))
    .extend({
      isValid: isValidPrim,
      ...
      defaultValue: assertValidPrim
    })
    .extend(methods);
}

function primCtor(name) {
  var env = { ctor: null };

  name = name[0].toUpperCase() + name.substr(1) + 'Type';

  with (env)
    eval('ctor = function ' + name + '(){ '
      + 'throw new Error("Cannot instantiate primitive `' + name + '` type.");'
      + '}'
    );
