...
Storage.prototype.create = function(obj, next) {
  ...
  function fail() {
    next(new Type.ValueError('Duplicate key `' + key + '` (' + tries + ' attempts)', obj));
  }
...
Storage.prototype.save = function(obj, next) {
  ...
  try {
    key = obj.beforeSave(false).__key__();
    data = type.dumpValid(obj);
  } catch (x) {
    error = x;
  }
  ...
  if (error)
    ...
    this.db.replace(key, data, function(err) {
      if (err && err.code == Kyoto.NOREC)
        next(new Type.ValueError("save: this object hasn't been created yet", obj));
      else if (err)
        next(err);
      else {
        try {
