...
IndexSet.include({
  ...
  addIndex: function(name, message) {
    return this.add(new Index(this.type, name, message));
  }
});
...
function Unique(type, fieldName, message) {
  indexField(this, type, fieldName);
...
Unique.include({
  ...
  key: function(obj) {
    var val = this.field.indexValue(obj[this.field.name]);
  ...
  generate: function(store, done) {
    ...
    return new Gen.TakeWhile(iter, this._prefixMatches);
  },
...
function indexField(index, type, fieldName) {
...
Type.create(Index);
function Index(type, fieldName, message) {
  indexField(this, type, fieldName);

  this.name = '#' + this.field.fullName();
  this.invalidMessage = message || 'index error';
}
