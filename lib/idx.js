...
function IndexSet(type) {
  this.type = type;
  this.indicies = null;
  this._match = new RegExp('^%' + Type.name(type) + '\\.([^{]+)');
}
...
IndexSet.include({
  ...
  matchIndex: function(name) {
    var probe = name.match(this._match);
...
Type.create(Index);
function Index(type, name, message, value) {
  var field = type.field(name);

  if (!field)
    throw new Error('No field called `' + name + '`.');
  ...
  if (field.primaryType && (fieldType = field.primaryType()))
    ...
    schema = type.schemaOf(name).type;
  ...
  this.deriveValue = value;
  this.invalidMessage = message || Type.of(this).defaultError;
}

Index.extend({
  defaultError: 'index error'
});

Index.include({
  prefix: function(val) {
    var prefix = '%' + this.fullName + '{';
    if (val !== undefined)
      prefix += val + '}';
    return prefix;
  },
  ...
  generate: function(store, value, done) {
    ...
    if (value === undefined)
      ...
      prefix = this.prefix(generateValue(this, value));
...
function Unique(type, name, message, value) {
  if (typeof message == 'function') {
    value = message;
    message = undefined;
  }
  Index.call(this, type, name, message, value);
}

Unique.extend({
  defaultError: 'duplicate value'
});
