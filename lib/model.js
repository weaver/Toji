...
Model.include({
  defaultStore: function() {
    return Type.of(this).defaultStore();
  },
  ...
  save: function(next) {
    this.defaultStore().save(this, next);
    return this;
  },

  remove: function(next) {
    this.defaultStore().remove(this, next);
    return this;
  },
  ...
  include: function() {
    ...
    Query.resolveRefs(this.defaultStore(), this, names, next);
    return this;
  }
});
...
Model.extend({
  ...
  findById: function(id, next) {
    this.defaultStore().findById(this, id, next);
    return this;
  }
});
...
Model.extend({
  ...
  emitAsync: function(name) {
    var self = this,
        events = this.__events__,
        listeners = this.__events__.listeners(name),
        args = U.toArray(arguments, 1),
        arity = args.length,
        done = args[arity - 1],
        error;

    U.aEach(listeners, done, function(fn, _, next) {
      if (fn.length < arity) {
        // The handler doesn't expect a callback, treat it
        // synchronously.
        try {
          args[arity - 1] = undefined;
          fn.apply(self, args);
        } catch (x) {
          error = x;
        }
        ...
        // The handler expects a callback, treat it asynchronously.
        args[arity - 1] = next;
        fn.apply(self, args);
      }
    });
...
NullableField.include({
  ...
  indexValue: function(val) {
    return this.exportJSONValue(val);
  },
