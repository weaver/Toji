...
NullableField.include({
  ...
  exportJSON: function(obj) {
    var self = this,
        primary = this.assumePrimary(obj);

    // if (this.fullName() == 'Box.point')
    //   console.log('EXPORT', )

    if (primary)
      return primary.exportJSON(obj);
    // else if (obj && (primary = this.primaryType()) && Type.isInstance(obj, primary))
    //   return primary.exportJSON(obj);
...
field(Key.ObjectId, IdField, Avro.Field);
